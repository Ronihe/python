...
# Using enums increase the level of abstraction and lets the programmer think about what the values mean rather than worry about how they are stored and accessed.Jul 3, 2019
# https://stackoverflow.com/questions/4709175/what-are-enums-and-why-are-they-useful
# the var can only be one out of a small set of possible values
...
# various people that interact with the sysytem
...
class Member(Account):
    ...
    def checkout_book_item(self, book_item):
        if self.get_total_books_checkedout(
        ...
        if book_reservation != None and book_reservation.get_member_id(
        ) != self.get_id():
    ...
    def renew_book_item(self, book_item):
        ...
        book_item.update_due_date(datetime.datetime.now().AddDays(
            Constants.MAX_LENDING_DAYS))
        return True
...
class BookReservation:
    ...
    @classmethod
    def fetch_reservation_details(cls, barcode):
        return "barcode"
...
class BookItem(Book):
    def __init__(self, barcode, is_reference_only, borrowed, due_date, price,
                 book_format, status, date_of_purchase, publication_date,
                 placed_at):
